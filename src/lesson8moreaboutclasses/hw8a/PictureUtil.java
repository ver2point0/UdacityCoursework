package lesson8moreaboutclasses.hw8a;//In this exercise we are going to manipulate images with the Picture class. We are
...
public class PictureUtil
{
    ...
    public static Picture grayAndFlipLeftToRight( Picture pic)
    {
        ...
        for (int i = 0; i < pixels.length; i++) {
            for (int j = 0; j < pixels[i].length; j++) {
                int temp = pixels[i][0];
                for (int e = 1; e < pixels[i].length - j; e++)  {
                    pixels[i][e - 1] = pixels[i][e];
                }
    ...
    public static Picture grayAndRotate90( Picture pic)
    {
        ...
        int[][] grayPic = pic.getGrayLevels();
        int row = grayPic.length;
        int column = grayPic[0].length;
        int[][] newArray = new int[column][row];
        for (int i = 0; i < newArray.length; i++) {
            for (int j = 0; j < newArray[0].length; j++) {
                newArray[i][j] = grayPic[j][i];
            }
        }
        Picture newArrayPic = new Picture(newArray);
        return newArrayPic;
    }
