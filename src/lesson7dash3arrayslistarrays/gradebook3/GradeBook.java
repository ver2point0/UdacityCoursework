...
public class GradeBook
{
     ...
     * The order of the topics matches the order of the topic names in
     * topicNames.
     * @return a filled array of the total for each topic.
     */
    public double[] totalsForAllTopics()
    {
        // Implement the totalsForAllTopics method.
        double[] totals = new double[numTopics];
        for (int topic = 0; topic < numTopics; topic++) {
            double topicSum = 0;
            for (int student = 0; student < numStudents; student++) {
                topicSum = topicSum + grades[student][topic];
            }
            totals[topic] = topicSum;
        }
        return totals;
    }
